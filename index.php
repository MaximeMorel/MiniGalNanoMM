...
// original photos dir
$photosdir = rtrim('photos' . "/" .$_REQUEST["dir"],"/");
$photosdir = str_replace("/..", "", $photosdir); // Prevent looking at any up-level folders

// reduced photos dir
$photosminidir = rtrim('photos_mini' . "/" .$_REQUEST["dir"],"/");
$photosminidir = str_replace("/..", "", $photosminidir); // Prevent looking at any up-level folders

// thumbnails dir
$thumbdir = rtrim('photos_thumb' . "/" .$_REQUEST["dir"],"/");
$thumbdir = str_replace("/..", "", $thumbdir); // Prevent looking at any up-level folders

$currentdir = GALLERY_ROOT . $photosdir;

//echo "photosdir : ".$photosdir."<br/>";
//echo "photosminidir : ".$photosminidir."<br/>";
    ...
    while (false !== ($file = readdir($handle))) {
        ...
        if (is_directory($currentdir . "/" . $file)) {
            if ($file != "." && $file != ".." ) {
                checkpermissions($currentdir . "/" . $file); // Check for correct file permission
                    // mkdirs
                    //echo "mkdir";
                    if(!file_exists($photosminidir."/".$file)) {
                    //echo "mkdir";
                        mkdir($photosminidir."/".$file, 0755, true);
                    }
                    if(!file_exists($thumbdir."/".$file)) {
                    //echo "mkdir";
                        mkdir($thumbdir."/".$file, 0755, true);
                    }

                    // Set thumbnail to folder.jpg if found:
                    if (file_exists("$currentdir/" . $file . "/folder.jpg")) {
                    //createthumb.php?filename=$currentdir/" . $file . "/folder.jpg&amp;size=$thumb_size'
                        genThumb($currentdir."/".$file."/folder.jpg", $thumbdir . "/" . $file);
                        $dirs[] = array(
                            "name" => $file,
                            "date" => filemtime($currentdir . "/" . $file . "/folder.jpg"),
                            "html" => "<li><a href='?dir=" .ltrim($_GET['dir'] . "/" . $file, "/") . "'><em>" . padstring($file, $label_max_length) . "</em><span></span><img src='" . GALLERY_ROOT . "$thumbdir/".$file."/folder.jpg' alt='$label_loading' /></a></li>\n");
                    }  else {
                    // Set thumbnail to first image found (if any):
                        unset ($firstimage);
                        $firstimage = getfirstImage("$currentdir/" . $file);
                        if ($firstimage != "") {
                    //createthumb.php?filename=$photosdir/" . $file . "/" . $firstimage . "&amp;size=$thumb_size'
                            genThumb($photosdir."/".$file."/".$firstimage, $thumbdir . "/" . $file."/".$firstimage);
                            $dirs[] = array(
                                "name" => $file,
                                "date" => filemtime($currentdir . "/" . $file),
                                "html" => "<li><a href='?dir=" . ltrim($_GET['dir'] . "/" . $file, "/") . "'><em>" . padstring($file, $label_max_length) . "</em><span></span><img src='" . GALLERY_ROOT . "$thumbdir/" . $file . "/" . $firstimage."'' alt='$label_loading' /></a></li>");
                        } else {
        ...
        if ($file != "." && $file != ".." && $file != "folder.jpg") {
            ...
            if (preg_match("/.jpg$|.gif$|.png$/i", $file)) {
                ...
                //createthumb.php?filename=" . $photosdir . "/" . $file . "&amp;size=$thumb_size'
                genMini($photosdir . "/" . $file, $photosminidir . "/" . $file);
                genThumb($photosminidir . "/" . $file, $thumbdir . "/" . $file);
                $files[] = array (
                    "name" => $file,
                    "date" => filemtime($currentdir . "/" . $file),
                    "size" => filesize($currentdir . "/" . $file),
                    "html" => "<li><a href='" . $photosminidir . "/" . $file . "' rel='lightbox[billeder]' title='$img_captions[$file]'><span></span><img src='" . GALLERY_ROOT . "$thumbdir" . "/" . $file . "' alt='$label_loading' /></a></li>");
            }
            // Other filetypes
            $extension = "";
